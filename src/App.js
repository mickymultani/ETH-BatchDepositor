...
function App() {
  ...
  const onContinue = async () => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        setCurrentStep(2); // Setting the current step to 2 to advance to the next screen.
  ...
  //const onNext = () => setCurrentStep((prevStep) => prevStep + 1);
  ...
  <div className="summary-section">
    ...
    <p>Abyss Contract: {summary.contractAddress}</p></div>
    ...
    <button onClick={() => setCurrentStep(1)} className="btn back-btn">Back</button>
  </div>
)}
